			...
			if (f) {
				...
				r.onload = (e) => {
					...
					workbook.SheetNames.forEach(function (sheetName) {
						const header = []
						const columnCount = XLSX.utils.decode_range(workbook.Sheets[sheetName]['!ref']).e.c + 1

						for (let i = 0; i < columnCount; ++i) {
							header[i] = workbook.Sheets[sheetName][`${XLSX.utils.encode_col(i)}1`].v
						}
...
function http(url, options) {
	const baseURL = process.env.VUE_APP_API_BASE_URI ? process.env.VUE_APP_API_BASE_URI : 'https://booqnly-stockr.herokuapp.com'
